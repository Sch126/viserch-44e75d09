...
Provide the original scientific proof for each concept.`;
    ...
    // Read PDF content (convert to base64 for AI processing)
    ...
    if (!dimensionXResponse.ok) {
      ...
      console.error("Dimension X API error:", errorText);
      throw new Error("Failed to process Dimension X");
    }

    const dimensionXData = await dimensionXResponse.json();
    const dimensionXContent = dimensionXData.choices?.[0]?.message?.content || "[]";
    ...
    if (!dimensionYResponse.ok) {
      console.error("Dimension Y API error:", await dimensionYResponse.text());
      throw new Error("Failed to process Dimension Y");
    }
    ...
    try {
      ...
      storyboard = [{
        ...
        narration: dimensionYContent.slice(0, 200)
      }];
    }
    ...
    if (!dimensionZResponse.ok) {
      console.error("Dimension Z API error:", await dimensionZResponse.text());
      throw new Error("Failed to process Dimension Z");
    }
    ...
    // Score based on: shorter segments = higher focus score
    // Ideal segment is ~150 chars (about 20 seconds of narration)
